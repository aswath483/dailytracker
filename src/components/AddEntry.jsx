...
function Celebration() {
  const items = Array.from({ length: 12 }, (_, i) => ({
    emoji: CELEBRATE[i % CELEBRATE.length],
    left: `${5 + (i * 8) % 88}%`,
    delay: `${i * 0.07}s`,
    bottom: `${10 + (i % 4) * 12}%`,
  }));
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center" style={{ backgroundColor: 'rgba(0,0,0,0.35)', backdropFilter: 'blur(6px)' }}>
      {items.map((item, i) => (
        <span key={i} className="float-up" style={{ left: item.left, bottom: item.bottom, animationDelay: item.delay }}>{item.emoji}</span>
      ))}
...
export default function AddEntry() {
  ...
  function isValid() {
    ...
    if (tab === 'weight')   return weight && !isNaN(parseFloat(weight));
    return true;
  }
  ...
  async function handleSubmit(e) {
    ...
    if (tab === 'exercise') setCelebrate(true);
    else navigate('/');
    setLoading(false);
  }
    ...
    <div className="min-h-screen bg-slate-100" style={{ paddingBottom: 'calc(5rem + env(safe-area-inset-bottom))' }}>
      ...
      <div className="px-4 -mt-5">
        <div className="bg-white rounded-3xl shadow-sm border border-gray-100/80 p-5">
          ...
          {/* Tabs - 3 + 3 grid for 6 tabs */}
          <div className="grid grid-cols-3 gap-1.5 bg-slate-100 rounded-2xl p-1 mb-6">
            {TABS.map(t => (
              <button key={t.id} type="button" onClick={() => setTab(t.id)}
                className={`flex flex-col items-center py-2.5 rounded-xl text-xs font-bold transition-all gap-0.5 ${tab === t.id ? 'bg-white text-gray-800 shadow-sm' : 'text-gray-400'}`}>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
              ...
              <>
                <div>
                  ...
                  <div className="grid grid-cols-4 gap-2">
                    {EXERCISE_TYPES.map(ex => (
                      <button key={ex.id} type="button" onClick={() => setExType(ex.id)}
                        className={`flex flex-col items-center py-3 rounded-2xl border-2 transition-all active:scale-95 ${exerciseType === ex.id ? 'border-emerald-400 bg-emerald-50' : 'border-gray-100 bg-slate-50'}`}>
                ...
                <div>
                  ...
                  <div className="flex items-center bg-slate-50 rounded-2xl border-2 border-gray-100 px-4 py-4 gap-4">
                    <button type="button" onClick={() => setMinutes(m => Math.max(1, m - 5))} className="w-11 h-11 rounded-full bg-white border border-gray-200 text-gray-600 text-xl font-bold flex items-center justify-center shadow-sm active:scale-90">−</button>
                    <div className="flex-1 flex flex-col items-center">
                      <input type="number" value={minutes} onChange={e => setMinutes(Math.max(1, parseInt(e.target.value)||1))} className="w-24 text-center text-4xl font-black text-emerald-600 focus:outline-none bg-transparent" />
                      <span className="text-emerald-400 font-bold text-xs uppercase tracking-wide">minutes</span>
                    </div>
                    <button type="button" onClick={() => setMinutes(m => Math.min(300, m + 5))} className="w-11 h-11 rounded-full bg-emerald-500 text-white text-xl font-bold flex items-center justify-center shadow-sm active:scale-90">+</button>
                  </div>
                  <div className="flex gap-2 mt-2.5 flex-wrap">
                    {[15,30,45,60,90].map(m => (
                      <button key={m} type="button" onClick={() => setMinutes(m)} className={`px-3.5 py-1.5 rounded-full text-xs font-bold transition-all active:scale-95 ${minutes===m?'bg-emerald-500 text-white shadow-sm':'bg-slate-100 text-gray-500'}`}>{m}m</button>
                    ))}
                  </div>
                </div>
              </>
            ...
            {tab === 'mood' && (
              <>
                  ...
                  <div className="flex justify-between bg-slate-50 rounded-2xl border-2 border-gray-100 p-4">
                    {MOODS.map(m => (
                      <button key={m.level} type="button" onClick={() => setMoodLevel(m.level)}
                        className={`flex flex-col items-center gap-1 px-2 py-2 rounded-xl transition-all active:scale-90 ${moodLevel===m.level?'bg-amber-50 scale-110':''}`}>
                ...
                <textarea value={moodNote} onChange={e => setMoodNote(e.target.value)} placeholder="Add a note (optional)..." rows={3} className="w-full px-4 py-3 rounded-2xl border-2 border-gray-100 bg-slate-50 text-gray-700 focus:outline-none focus:border-violet-400 resize-none text-sm font-medium" />
              </>
            )}
            ...
            {tab === 'note' && (
              <div>
                ...
                <textarea value={note} onChange={e => setNote(e.target.value)} placeholder="Write anything about your day..." rows={5} autoFocus className="w-full px-4 py-3 rounded-2xl border-2 border-gray-100 bg-slate-50 text-gray-700 focus:outline-none focus:border-violet-400 resize-none text-sm font-medium leading-relaxed" />
                <button type="button" onClick={() => setIsPrivate(p => !p)}
                  className={`mt-2 flex items-center gap-2 px-4 py-2.5 rounded-2xl border-2 w-full transition-all ${isPrivate ? 'border-violet-300 bg-violet-50' : 'border-gray-100 bg-slate-50'}`}>
                  <span className="text-lg">{isPrivate ? '🔒' : '🌐'}</span>
                  <span className={`text-sm font-bold ${isPrivate ? 'text-violet-700' : 'text-gray-500'}`}>
                    {isPrivate ? 'Private — only you can see this' : 'Shared — partner can see this'}
                  </span>
                </button>
              </div>
            )}
            ...
            {tab === 'sleep' && (
              <div>
                ...
                <div className="flex items-center bg-slate-50 rounded-2xl border-2 border-gray-100 px-4 py-4 gap-4">
                  <button type="button" onClick={() => setHours(h => Math.max(1, h - 0.5))} className="w-11 h-11 rounded-full bg-white border border-gray-200 text-gray-600 text-xl font-bold flex items-center justify-center shadow-sm active:scale-90">−</button>
                  <div className="flex-1 flex flex-col items-center">
                  ...
                  <button type="button" onClick={() => setHours(h => Math.min(14, h + 0.5))} className="w-11 h-11 rounded-full bg-indigo-500 text-white text-xl font-bold flex items-center justify-center shadow-sm active:scale-90">+</button>
                </div>
                <div className="flex gap-2 mt-2.5 flex-wrap">
                  {[5,6,7,7.5,8,9].map(h => (
                    <button key={h} type="button" onClick={() => setHours(h)} className={`px-3.5 py-1.5 rounded-full text-xs font-bold transition-all active:scale-95 ${hours===h?'bg-indigo-500 text-white shadow-sm':'bg-slate-100 text-gray-500'}`}>{h}h</button>
                  ))}
                </div>
              </div>
            )}
            ...
            {tab === 'weight' && (
              <div>
                <p className="text-xs font-extrabold text-gray-400 uppercase tracking-widest mb-3">Your weight</p>
                <div className="flex gap-2 mb-3">
                  {['kg', 'lbs'].map(u => (
                    <button key={u} type="button" onClick={() => setUnit(u)}
                      className={`flex-1 py-2.5 rounded-2xl text-sm font-bold border-2 transition-all ${weightUnit===u?'border-rose-400 bg-rose-50 text-rose-700':'border-gray-100 bg-slate-50 text-gray-500'}`}>
                      {u}
                    </button>
                  ))}
                </div>
                <div className="flex items-center bg-slate-50 rounded-2xl border-2 border-gray-100 px-4 py-4">
                  <input
                    type="number"
                    step="0.1"
                    value={weight}
                    ...
                    placeholder="0.0"
                    autoFocus
                    className="flex-1 text-center text-4xl font-black text-rose-600 focus:outline-none bg-transparent"
                  />
                  <span className="text-rose-400 font-bold text-lg">{weightUnit}</span>
                </div>
              </div>
            )}
